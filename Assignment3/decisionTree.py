...
class decesionTree:
    ...
    def buildTree(self, trnData, treeNode, cl, cr):
        ...
        fcbll, fcblr, fcbrl, fcbrr = 0, 0, 0, 0

        for idx in range(1, trnData.shape[1]):
            ftrColName = list(trnData)[idx]
            threshold, giniNum, cbll, cblr, cbrl, cbrr = \
                self.getMaxThresholdInfo(cl, cr, trnData.sort_values(by=[ftrColName]), ftrColName)

            if giniNum > maxGiniNum:
                fFtrColName, fThreshold, maxGiniNum = ftrColName, threshold, giniNum
                fcbll, fcblr, fcbrl, fcbrr = cbll, cblr, cbrl, cbrr
    ...
    def getMaxThresholdInfo(self, cl, cr, df, ftrColName):
        ...
        fcbll, fcblr, fcbrl, fcbrr = 0, 0, 0, 0
        threshold, maxGiniNum = 0, 0.0
        resultVal = df.iloc[0, 0]

        for i, row in df.iloc[1:].iterrows():
            preResultVal = resultVal
            cbll, cblr, cbrl, cbrr = self.computeFirstResult(resultVal, cbll, cblr, cbrl, cbrr)
            resultVal = row[0]

            if preResultVal != resultVal:
                giniNum = self.giniValue(cl, cr, cbll, cblr, cbrl, cbrr) * self.dataWeights[i]
                if giniNum > maxGiniNum:
                    threshold, maxGiniNum = row[ftrColName], giniNum
                    fcbll, fcblr, fcbrl, fcbrr = cbll, cblr, cbrl, cbrr

        return threshold, maxGiniNum, fcbll, fcblr, fcbrl, fcbrr

    def giniValue(self, cl, cr, cbll, cblr, cbrl, cbrr):
        vl = float((2 * cbll * cblr) / ((cbrl + cbrr) * (cl + cr)))
        vr = float((2 * cbrl * cbrr) / ((cbrl + cbrr) * (cl + cr)))
        return 1.0 - vl - vr

    def computeFirstResult(self, resultVal, cbll, cblr, cbrl, cbrr):
        if resultVal == 1:
            cbll += 1
            cbrl -= 1
        else:
    ...
    def predictRec(self, data, treeNode):
        if data.shape[0] == 0:
            return
        ...
        self.predictRec(branchLeft, treeNode.left)
        self.predictRec(branchRight, treeNode.right)
